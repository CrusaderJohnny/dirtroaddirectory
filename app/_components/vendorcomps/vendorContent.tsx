...
export default function VendorsContent() {
            ...
            if (user) {
                try {
                    ...
                    if (matchedUser) {
                        ...
                        const favVendorIds = await favoriteVendorsAPI.getFavoriteVendorIds(dbId);
                        setFavoriteVendorIds(favVendorIds);
                    } else {
                        console.warn("No DB user matched for Clerk email.");
                    }
                } catch (favErr) {
                    console.warn("Failed to fetch favorite vendors:", favErr);
                    // Do NOT set error — just log
                }
            }
    ...
    const toggleFavoriteVendor = async (vendorId: number) => {
        ...
        const isFav = favoriteVendorIds.includes(idStr);

        try {
            if (isFav) {
                await favoriteVendorsAPI.removeFavoriteVendor(dbUserId, idStr);
                setFavoriteVendorIds(prev => prev.filter(id => id !== idStr));
            } else {
                await favoriteVendorsAPI.addFavoriteVendor(dbUserId, idStr);
                setFavoriteVendorIds(prev => [...prev, idStr]);
            }
        } catch (err) {
            ...
            <AppShellMain style={{minHeight: "100vh"}}>
                <Container size="lg" py="xl">
                    ...
                    <motion.div initial="hidden" whileInView="visible" viewport={{once: true}} variants={fadeInUp}>
                        <Card withBorder shadow="lg" radius="md" p={0} mb="xl"
                    ...
                    <motion.div initial="hidden" whileInView="visible" viewport={{once: true}} variants={fadeInUp}>
                        <Card withBorder radius="md"
                              style={{backgroundColor: "#fff", marginBottom: "1.5rem", padding: "1.5rem"}}>
                            <Group align="center" mb="sm">
                                <ThemeIcon variant="light" color="orange" radius="xl" size="lg"><IconList
                                    size={20}/></ThemeIcon>
                    ...
                    <motion.div initial="hidden" whileInView="visible" viewport={{once: true}} variants={fadeInUp}>
                        <Card withBorder radius="md"
                              style={{backgroundColor: "#fff", marginBottom: "1.5rem", padding: "1.5rem"}}>
                            <Group align="center" mb="sm">
                                <ThemeIcon variant="light" color="green" radius="xl" size="lg"><IconShoppingCart
                                    size={20}/></ThemeIcon>
                    ...
                    <motion.div initial="hidden" whileInView="visible" viewport={{once: true}} variants={fadeInUp}>
                        <Card withBorder radius="md"
                              style={{backgroundColor: "#ffffff", marginBottom: "1.5rem", padding: "1.5rem"}}>
                                ...
                                <Grid gutter="lg">
                                    {associatedMarkets.map((market) => ( // Changed 'market, idx' to just 'market'
                                        <Grid.Col span={{base: 12, sm: 6, md: 4}}
                                                  key={market.id}> {/* Use market.id as key */}
                                            <Link href={`/markets?marketId=${market.id}`} passHref>
                                                <Card withBorder radius="md" p="md"
                                                      style={{backgroundColor: "#f9fafb", cursor: "pointer"}}>
                                                    <Group mb="xs" gap="xs">
                                                        <ThemeIcon variant="light" c="red" size="sm"><IconMapPin
                                                            size={16}/></ThemeIcon>
                                                        <Text fw={600} size="md"
                                                              c="blue">{market.label}</Text> {/* Changed to market.label */}
                                                    </Group>
                        ...
                        <Grid gutter="xl">
                            <Grid.Col span={{base: 12, sm: 6}}>
                                <Card shadow="sm" radius="md" withBorder p="lg" bg="#e6f4ea">
                                    ...
                                    <Text size="sm"><strong>Phone:</strong> {selectedVendor.contact || 'Not available'}
                                    </Text>
                                    <Text size="sm"><strong>Email:</strong> {selectedVendor.email ? (
                                        <a href={`mailto:${selectedVendor.email}`}
                                           style={{color: '#1e88e5'}}>{selectedVendor.email}</a>
                                    ) : 'Not available'}</Text>
                                    <Text size="sm"><strong>Website:</strong> {selectedVendor.website ? (
                                        <a href={selectedVendor.website} target="_blank" rel="noopener noreferrer"
                                           style={{color: '#1e88e5'}}>{selectedVendor.website}</a>
                                    ) : 'Not available'}</Text>
                                </Card>
                            </Grid.Col>
                            ...
                            <Grid.Col span={{base: 12, sm: 6}}>
                                <Card shadow="sm" radius="md" withBorder p="lg" bg="#d3d3d3">
                                    <Group justify="center" mb="sm">
                                        <ThemeIcon size="xl" radius="xl" color="gray"><IconShare2
                                            size={28}/></ThemeIcon>
                                    </Group>
                                    <Title order={4} ta="center" c="dark.7">SOCIAL MEDIA</Title>

                                    <Group justify="space-evenly" mt="md">
                                        <Group gap={6}><IconBrandFacebook color="darkblue"/><Text
                                            size="sm">Facebook</Text></Group>
                                        <Group gap={6}><IconBrandInstagram color="purple"/><Text
                                            size="sm">Instagram</Text></Group>
                                    </Group>
                                </Card>
                            </Grid.Col>
                        </Grid>
                    ...
                    <Group justify="center" mt="xl">
                        <Button component="a" href="/vendors" variant="outline" color="orange">← Back to all
                            Vendors</Button>
                    </Group>
                </Container>
            </AppShellMain>
    ...
    return (
        <AppShellMain style={{minHeight: "100vh"}}>
            ...
            <Grid gutter="xl">
                {filteredVendors.map((vendor: VendorsInterface) => (
                    <Grid.Col span={{ base: 12, sm: 6, md: 4 }} key={vendor.id}>
                        <VendorCard
                            vendor={vendor}
                            {...(user && {
                                isFavorited: favoriteVendorIds.includes(vendor.id.toString()),
