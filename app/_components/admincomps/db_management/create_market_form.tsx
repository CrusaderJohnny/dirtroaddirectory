...
// Define the props for the MarketForm component
interface MarketFormProps {
  market?: MarketsInterface; // Optional: If provided, the form is in 'edit' mode
  onSave: () => void; // Callback after successful save (create/update)
  onCancel: () => void; // Callback for when the user cancels
}

export default function CreateMarketForm({ market, onSave, onCancel }: MarketFormProps) {
  const form = useForm({
  ...
  useEffect(() => {
    if (market) {
      form.setValues({
        image: market.image,
        label: market.label,
        description: market.description || '', // Use empty string for null/undefined to avoid React warnings
        content: market.description || '',
        link: market.link || '',
        lat: market.lat !== null ? Number(market.lat) : undefined, // Convert Decimal to Number, handle null
        lng: market.lng !== null ? Number(market.lng) : undefined, // Convert Decimal to Number, handle null
      });
    } else {
  ...
  const handleSubmit = async (values: typeof form.values) => {
    try {
      ...
      onSave(); // Call onSave callback to signal parent component to refresh/close form
    } catch (error) {
  ...
  return (
    <form onSubmit={form.onSubmit(handleSubmit)}>
      ...
      <Textarea
        label="Description"
        placeholder="A brief description of the market..."
      ...
      <Textarea
        label="Content"
        placeholder="Detailed information about the market..."
      ...
      <Group justify="flex-end" mt="xl"> {/* Increased margin for buttons */}
        ...
        <Button type="submit">{market && market.id ? 'Update Market' : 'Create Market'}</Button>
      </Group>
    </form>
  );
}