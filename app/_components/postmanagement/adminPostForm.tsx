...
import {AdminPostFormProps, MarketOption, VendorOption} from "@/app/_types/interfaces";
...
export default function AdminPostForm({user} : AdminPostFormProps) {
    ...
    const getPosterName = () => {
        ...
        if(selectedPosterType === 'market'){
            ...
            const vendor = vendorOptions.find(v => v.id === selectedPosterID);
            ...
            <form onSubmit={form.onSubmit(handleSubmit)}>
                ...
                {selectedPosterType === 'vendor' && (
                    <Select
                        ...
                        data={vendorOptions.map(v => ({ value: v.id, label: v.name }))}
                ...
                {selectedPosterType === 'market' && (
                    <Select
                        ...
                        data={marketOptions.map(m => ({ value: m.id, label: m.name }))}
                        value={selectedPosterID}
                        onChange={setSelectedPosterID}
                        searchable
                        clearable
