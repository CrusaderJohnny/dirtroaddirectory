 ...
 * @param request The NextRequest object (not directly used for query params, but good practice for route handlers).
 * @param params Contains the dynamic 'id' (user ID) from the URL.
 */
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) { // Changed params type to Promise
 ...
 * @param params Contains the dynamic 'id' (user ID) from the URL.
 */
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) { // Changed params type to Promise
    const { id: userId } = await params; // Await params here!
