...
export async function GET(request: NextRequest, { params }: { params: Promise<{ uuid: string }> }) {
    ...
    try {
        ...
        */
        ...
        const response = await fetch(`${BACKEND_API_BASE_URL}/vendors/uuid/${uuid}`); // Call backend UUID endpoint
        if (!response.ok) {
        ...
        console.error(`Error in Next.js API route (GET /api/vendors/uuid/${uuid}):`, error);
